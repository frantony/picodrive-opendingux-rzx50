...
static int init_console(void)
{
   ...
   if (__al_linux_vt != 0) {
	 ...
	 if (!(vts.v_state & mask)) {
      ...
      if (!mask) {
	 printf("Unable to find a usable VT\n");
	 close (console_fd);
	 return 1;
      }
	 ...
	 exit (0);
      ...
      if (fd == -1) {
	 printf("Unable to reopen new console\n");
	 return 1;
      }
...
{
    if (init_console()) {
      printf("YYYYYYYYYYYYYYYYYYEEEEEEEEEEEEEEESSSSSSSSSSSSSSSS\n");
      return 1;
   }
...
{
   if (__al_linux_use_console()) ioctl(__al_linux_console_fd, KDSETMODE, KD_GRAPHICS);
}
